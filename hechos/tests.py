...
from hechos.models import Curso, EdicionCurso, Escuela, Estudiante, Matricula, Profesor, RutaEstudio, SolicitudMatricula
...
class SolicitudMatriculaFlowTests(TestCase):
    def setUp(self):
        ...
        self.estudiante = Estudiante.objects.create(user=self.student_user)
    ...
    def test_duplicate_request_is_not_created(self):
        SolicitudMatricula.objects.create(
            ...
            edicion_curso=self.edicion,
        )
        self.client.force_login(self.student_user)

        self.client.post(reverse("hechos:solicitar_matricula", args=[self.edicion.id]))

        self.assertEqual(
            SolicitudMatricula.objects.filter(estudiante=self.estudiante, edicion_curso=self.edicion).count(),
            1,
        )
    ...
    def test_student_with_active_matricula_cannot_create_request(self):
        ...
        self.assertFalse(
            SolicitudMatricula.objects.filter(estudiante=self.estudiante, edicion_curso=self.edicion).exists()
        )
